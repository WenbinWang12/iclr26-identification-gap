#!/bin/bash
# Phase-2Y-A: Two-stage (PSR + SCG) development sweep
#SBATCH -J ya_dev
#SBATCH -p preempt
...
#SBATCH -t 04:00:00
#SBATCH -a 1-3
#SBATCH -o /home/pengq/iclr26/ya_dev_%A_%a.out
#SBATCH -e /home/pengq/iclr26/ya_dev_%A_%a.err

set -x

SEED=$SLURM_ARRAY_TASK_ID

echo "PHASE-2Y-A DEV: Two-stage (PSR+SCG) seed=$SEED"
...
OUT=/home/pengq/iclr26/results/y2stage_dev_s${SEED}
...
$V experiments/phase2y_composite/run_two_stage.py \
  ...
  --psr-cover-first 4 --psr-cover-rest 12 --psr-budget 16 \
  --scg-tau-grid "0.8,1.0,1.2,1.4,1.6,2.0" --scg-worst-threshold -1.0 \
  --risk-per-class 32 --audit-per-class 32 --eval-batch-size 4

EXIT=$?
echo "YA_DEV_DONE seed=$SEED exit=$EXIT"
exit $EXIT
