#!/bin/bash
# Phase-2Z-C: Combined two-constraint method (grouping + per-scope offset)
# 2x2 at matched fixed budget: {shared, grouped} x {no offset, +per-scope offset}
#SBATCH -J z_combo
#SBATCH -p preempt
...
#SBATCH -t 08:00:00
#SBATCH -a 1-3
#SBATCH -o /home/pengq/iclr26/z_combo_%A_%a.out
#SBATCH -e /home/pengq/iclr26/z_combo_%A_%a.err

set -x
SEED=$SLURM_ARRAY_TASK_ID
echo "PHASE-2Z-C: Combined method seed=$SEED (shared r8 vs 4x r2 + per-scope offset, epochs=7)"
...
OUT=/home/pengq/iclr26/results/z_combo_s${SEED}

$V experiments/phase2z_task_grouping/run_combined.py \
...
EXIT=$?
echo "Z_COMBO_DONE seed=$SEED exit=$EXIT"
exit $EXIT
